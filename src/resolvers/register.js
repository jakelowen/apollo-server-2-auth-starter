...
const register = isNotAuthenticatedResolver.createResolver(
  async (root, {
    ...
    // PRE. Check if this user already exists
    const existingUser = await dataLoaders.user.byEmail.load(email);

    if (existingUser) {
      return {
        code: 'alreadyExists',
        success: false,
        message: `User already exists for email ${email}. Try Logging in.`,
        securityCode: null,
      };
    }
